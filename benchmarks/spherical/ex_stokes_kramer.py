...
if "__file__" in globals():
    _repo_root_for_imports = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
    if _repo_root_for_imports not in sys.path:
        sys.path.insert(0, _repo_root_for_imports)

from analytical_solutions.kramer_spherical import (
    analytical_pressure_sympy,
    analytical_radial_stress_assess_sympy,
    analytical_velocity_cartesian_sympy,
    build_delta_solution,
    build_smooth_solution,
)
...
# %%
v_ana_above_sym = analytical_velocity_cartesian_sympy(soln_above, r_uw, y_lm_sym, th_uw, phi_raw, mesh.CoordinateSystem.rRotN)
v_ana_below_sym = analytical_velocity_cartesian_sympy(soln_below, r_uw, y_lm_sym, th_uw, phi_raw, mesh.CoordinateSystem.rRotN)
