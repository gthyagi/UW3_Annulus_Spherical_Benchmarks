...
res = uw.options.getInt("res", default=8) # 4, 8, 16, 32, 64, 96 
refine = None
...
if case in ('case1', 'case3'):
    msh_h5_file = f"./output/create_mesh/uw_spherical_shell_ro{r_o}_ri{r_i}_rint{r_int}_res{res}_refineNone.msh.h5"
    # msh_h5_file = f"{output_dir}mesh.msh.h5"
    ...
    msh_h5_file = f"./output/create_mesh/uw_spherical_shell_ro{r_o}_ri{r_i}_res{res}_refineNone.msh.h5"
...
mesh = uw.discretisation.Mesh(msh_h5_file, degree=1, qdegree=max(pdegree, vdegree), 
                              ...
                              useMultipleTags=True, useRegions=True, markVertices=True, 
                              boundaries=boundaries, boundary_normals=boundary_normals,
                              refinement=refine, verbose=0)
