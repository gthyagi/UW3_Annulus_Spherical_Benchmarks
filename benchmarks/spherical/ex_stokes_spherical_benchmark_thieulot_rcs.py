...
# whether to load mesh h5 or not
msh_h5_load = True
msh_h5_file = f"../../output/spherical/thieulot/meshes/uw_spherical_shell_ro{r_o}_ri{r_i}_res{res}.msh.h5"
# msh_h5_file = f"../output/create_mesh/uw_spherical_shell_ro{r_o}_ri{r_i}_res{res}_refine0.msh.h5"
...
# +
# output_dir 
output_dir = os.path.join(os.path.join("../../output/spherical/thieulot/legacy/"), 
                          f'model_m_{m}_res_{res}_vdeg_{vdegree}_pdeg_{pdegree}'\
                          f'_pcont_{pcont_str}_vel_penalty_{vel_penalty_str}_stokes_tol_{stokes_tol_str}/')
output_dir_chpt = os.path.join(os.path.join("../../output/spherical/thieulot/legacy/Checkpoint/"), 
                               f'model_m_{m}_res_{res}_vdeg_{vdegree}_pdeg_{pdegree}'\
                               f'_pcont_{pcont_str}_vel_penalty_{vel_penalty_str}_stokes_tol_{stokes_tol_str}/')
...
#                           f'model_m_{m}_res_{res}_vdeg_{vdegree}_pdeg_{pdegree}'\
#                           f'_pcont_{pcont_str}_vel_penalty_{vel_penalty_str}_stokes_tol_{stokes_tol_str}/')
# output_dir_chpt = os.path.join(os.path.join("./output/"), 
#                                f'model_m_{m}_res_{res}_vdeg_{vdegree}_pdeg_{pdegree}'\
#                                f'_pcont_{pcont_str}_vel_penalty_{vel_penalty_str}_stokes_tol_{stokes_tol_str}/')

if uw.mpi.rank == 0:
    os.makedirs(output_dir, exist_ok=True)
    os.makedirs(output_dir_chpt, exist_ok=True)
# -
...
if msh_h5_load:
    ...
    mesh = uw.discretisation.Mesh(msh_h5_file, degree=1, qdegree=max(pdegree, vdegree), 
...
if analytical:
    with mesh.access(v_ana, p_ana, rho_ana):
        ...
        p_ana.data[:,0] = uw.function.evaluate(p.subs({r:r_uw, theta:th_uw, phi:phi_uw}), p_ana.coords)
        rho_ana.data[:,0] = uw.function.evaluate(rho.subs({r:r_uw, theta:th_uw}), rho_ana.coords)
            
        v_ana.data[:,0] = uw.function.evaluate(v_x.subs({r:r_uw, theta:th_uw, phi:phi_uw}), v_ana.coords)
        v_ana.data[:,1] = uw.function.evaluate(v_y.subs({r:r_uw, theta:th_uw, phi:phi_uw}), v_ana.coords)
        v_ana.data[:,2] = uw.function.evaluate(v_z.subs({r:r_uw, theta:th_uw, phi:phi_uw}), v_ana.coords)
...
mesh.petsc_save_checkpoint(index=0, meshVars=[v_uw, p_uw, v_ana, p_ana, v_err, p_err], outputPath=os.path.relpath(output_dir_chpt)+'/output')
