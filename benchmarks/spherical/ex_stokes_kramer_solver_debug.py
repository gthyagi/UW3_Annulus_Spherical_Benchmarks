...
params = uw.Params(
    ...
    uw_solver_diagnostics=uw.Param(
        True,
        type=uw.ParamType.BOOLEAN,
        description="Enable verbose PETSc KSP/PC/SNES diagnostics for Stokes solve debugging",
    ),
    uw_solver_fail_fast=uw.Param(
        True,
        type=uw.ParamType.BOOLEAN,
...
if params.uw_solver_diagnostics:
    stokes.petsc_options["snes_monitor"] = None
    stokes.petsc_options["snes_converged_reason"] = None
    stokes.petsc_options["ksp_converged_reason"] = None
    stokes.petsc_options["ksp_error_if_not_converged"] = None
    stokes.petsc_options["pc_view"] = None
    stokes.petsc_options["ksp_view"] = None
    stokes.petsc_options["snes_view"] = None
    stokes.petsc_options["options_left"] = None
...
if checkpoint_mode:
    ...
    if uw.mpi.rank == 0:
        print("=== Stokes Solver Debug Summary ===", flush=True)
        print(f"snes_converged_reason: {snes_reason}", flush=True)
        print(f"ksp_converged_reason: {ksp_reason}", flush=True)
        print(f"snes_iterations: {snes_iterations}", flush=True)
        print(f"ksp_iterations: {ksp_iterations}", flush=True)

    if params.uw_solver_fail_fast and (snes_reason < 0 or ksp_reason < 0):
        raise RuntimeError(
            "Stokes solve failed; refusing to write benchmark output. "
            f"snes_reason={snes_reason}, ksp_reason={ksp_reason}, "
            f"snes_iterations={snes_iterations}, ksp_iterations={ksp_iterations}."
        )
