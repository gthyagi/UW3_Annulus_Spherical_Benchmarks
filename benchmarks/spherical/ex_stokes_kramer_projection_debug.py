...
params = uw.Params(
    ...
    uw_metrics_from_checkpoint_only=uw.Param(
        False,
        type=uw.ParamType.BOOLEAN,
        description="Reload saved Velocity/Pressure checkpoint fields and compute only projection debug metrics",
    ),
    uw_projection_debug_methods=uw.Param(
        "all",
        type=uw.ParamType.STRING,
        description=(
            "Comma-separated projection debug methods: all, "
            "current_scalar_lightweight, multicomponent_default, "
            "multicomponent_lightweight, stokes_tau"
        ),
    ),
    uw_bc_type=uw.Param(
        None,
...
metrics_filename = "benchmark_projection_debug_metrics.h5"
...
if not checkpoint_mode:
    ...
    if uw.mpi.rank == 0:
        print(
            ...
            "to compute benchmark_projection_debug_metrics.h5 from the saved checkpoint."
        )
    raise SystemExit(0)
...
def maxrss_mib():
    """Return per-rank high-water RSS in MiB."""
...
def memory_summary():
    """Return max and sum high-water RSS across MPI ranks."""
    local_rss_mib = maxrss_mib()
    comm = MPI.COMM_WORLD
    return {
        "rss_max_mib": float(comm.allreduce(local_rss_mib, op=MPI.MAX)),
        "rss_sum_mib": float(comm.allreduce(local_rss_mib, op=MPI.SUM)),
    }


def apply_l2_projection_solver_options(projector, tolerance):
    """Use lightweight options for pure L2 projection postprocessing."""
...
def symmetric_tau_indices(dim):
    """Return independent upper-triangular indices for a symmetric tensor."""
    return [(i, j) for i in range(dim) for j in range(i, dim)]


def project_tau_component(mesh, tau_expr, i, j, degree, tolerance, method_name, lightweight):
    """Project one deviatoric-stress component and keep projector lifetime local."""
    tau_component = uw.discretisation.MeshVariable(
        varname=f"{method_name}_Tau{i}{j}",
        mesh=mesh,
        degree=degree,
        vtype=uw.VarType.SCALAR,
    ...
    try:
        ...
        if lightweight:
            apply_l2_projection_solver_options(projector, tolerance)

        uw.pprint(f"Stage start: {method_name} projecting tau_{i}{j}")
        projector.solve()
        uw.pprint(f"Stage complete: {method_name} projecting tau_{i}{j}")
    finally:
        del projector
        gc.collect()
...
# %%
def project_symmetric_tau_scalar(mesh, tau_expr, degree, tolerance, method_name, lightweight):
    """Project symmetric deviatoric-stress components as separate scalar solves."""
    tau_projected_components = {}

    for i, j in symmetric_tau_indices(mesh.dim):
...
def project_symmetric_tau_multicomponent(
    mesh,
    tau_expr,
    degree,
    tolerance,
    ...
    """Project symmetric deviatoric-stress components in one multi-component solve."""
    indices = symmetric_tau_indices(mesh.dim)
    tau_flat = uw.discretisation.MeshVariable(
        f"{method_name}_TauFlat",
        mesh,
        (1, len(indices)),
    ...
    try:
        ...
        projector.uw_function = sp.Matrix([[tau_expr[i, j] for i, j in indices]])
        projector.smoothing = 0.0
        projector.tolerance = tolerance
        if lightweight:
            apply_l2_projection_solver_options(projector, tolerance)

        uw.pprint(f"Stage start: {method_name} projecting tau components")
        projector.solve()
        uw.pprint(f"Stage complete: {method_name} projecting tau components")
    finally:
        del projector
        gc.collect()

    tau_projected_expr = sp.MutableDenseMatrix.zeros(mesh.dim, mesh.dim)
    for component, (i, j) in enumerate(indices):
        tau_projected_expr[i, j] = tau_flat.sym[0, component]
        tau_projected_expr[j, i] = tau_flat.sym[0, component]

    return sp.Matrix(tau_projected_expr), {"tau_flat": tau_flat}


def project_symmetric_tau_stokes_tau(stokes):
    """Use UW3's built-in lazy tau projection path."""
    uw.pprint("Stage start: stokes_tau projecting tau")
    tau = stokes.tau
    uw.pprint("Stage complete: stokes_tau projecting tau")
    return sp.Matrix(tau.sym), {"tau": tau}


PROJECTION_DEBUG_METHODS = {
    "current_scalar_lightweight": {
        "label": "Current benchmark: six scalar Projection solves + lightweight L2 options",
        "lmoresi_requested": False,
    },
    "multicomponent_default": {
        "label": "lmoresi requested: MultiComponent_Projection with default solver options",
        "lmoresi_requested": True,
    },
    "multicomponent_lightweight": {
        "label": "lmoresi requested: MultiComponent_Projection + lightweight L2 options",
        "lmoresi_requested": True,
    },
    "stokes_tau": {
        "label": "UW3 built-in stokes.tau lazy projection path",
        "lmoresi_requested": False,
    },
}


def selected_projection_debug_methods(method_spec):
    """Parse requested projection debug methods."""
    if method_spec.strip().lower() == "all":
        return list(PROJECTION_DEBUG_METHODS)

    methods = [item.strip() for item in method_spec.split(",") if item.strip()]
    unknown = [method for method in methods if method not in PROJECTION_DEBUG_METHODS]
    if unknown:
        raise ValueError(
            f"Unknown projection debug method(s): {unknown}. "
            f"Valid methods: {list(PROJECTION_DEBUG_METHODS)}"
        )
    return methods


def project_tau_for_method(method_name):
    """Run one tau projection method and return projected tau expression plus holders."""
    tau_expr = sp.Matrix(stokes.stress_deviator)
    if method_name == "current_scalar_lightweight":
        return project_symmetric_tau_scalar(
            mesh=mesh,
            tau_expr=tau_expr,
            degree=params.uw_vdegree,
    ...
    if method_name == "multicomponent_default":
        return project_symmetric_tau_multicomponent(
            mesh=mesh,
            tau_expr=tau_expr,
            degree=params.uw_vdegree,
            tolerance=params.uw_stokes_tol,
    ...
    if method_name == "multicomponent_lightweight":
        return project_symmetric_tau_multicomponent(
            mesh=mesh,
            tau_expr=tau_expr,
            degree=params.uw_vdegree,
    ...
    if method_name == "stokes_tau":
        return project_symmetric_tau_stokes_tau(stokes)

    raise ValueError(f"Unhandled projection debug method: {method_name}")


def sigma_rr_error_from_tau(tau_projected_expr):
    """Return radial total-stress error from projected deviatoric stress."""
    sigma_rr_uw_sym = (n_vec.T * tau_projected_expr * n_vec)[0] - p_soln.sym[0]
    return sigma_rr_uw_sym - sigma_rr_ana_sym
...
def run_projection_debug_methods():
    """Run selected tau projection methods and return timing / metric diagnostics."""
    selected_methods = selected_projection_debug_methods(params.uw_projection_debug_methods)
    results = {}

    for method_name in selected_methods:
        method_info = PROJECTION_DEBUG_METHODS[method_name]
        gc.collect()
        MPI.COMM_WORLD.Barrier()
        memory_before = memory_summary()
        t_start = MPI.Wtime()

        tau_projected_expr, projection_holders = project_tau_for_method(method_name)

        MPI.COMM_WORLD.Barrier()
        projection_seconds = float(
            MPI.COMM_WORLD.allreduce(MPI.Wtime() - t_start, op=MPI.MAX)
        )
        memory_after_projection = memory_summary()

        sigma_rr_err_sym = sigma_rr_error_from_tau(tau_projected_expr)
        metric_start = MPI.Wtime()
        sigma_rr_lower = relative_l2_error(
            mesh, sigma_rr_err_sym, sigma_rr_ana_sym, boundary=lower
        )
        sigma_rr_upper = relative_l2_error(
            mesh, sigma_rr_err_sym, sigma_rr_ana_sym, boundary=upper
        )
        MPI.COMM_WORLD.Barrier()
        sigma_metric_seconds = float(
            MPI.COMM_WORLD.allreduce(MPI.Wtime() - metric_start, op=MPI.MAX)
        )
        memory_after_metrics = memory_summary()

        results[method_name] = {
            "label": method_info["label"],
            "lmoresi_requested": bool(method_info["lmoresi_requested"]),
            "projection_seconds": projection_seconds,
            "sigma_metric_seconds": sigma_metric_seconds,
            "rss_max_mib_before": memory_before["rss_max_mib"],
            "rss_max_mib_after_projection": memory_after_projection["rss_max_mib"],
            "rss_max_mib_after_metrics": memory_after_metrics["rss_max_mib"],
            "rss_sum_mib_after_metrics": memory_after_metrics["rss_sum_mib"],
            "sigma_rr_l2_norm_lower": float(sigma_rr_lower),
            "sigma_rr_l2_norm_upper": float(sigma_rr_upper),
        }

        del tau_projected_expr
        del projection_holders
        del sigma_rr_err_sym
        gc.collect()
...
projection_debug_results = run_projection_debug_methods()
primary_projection_method = next(iter(projection_debug_results), None)
if primary_projection_method is None:
    sigma_rr_err_l2_lower = np.nan
    sigma_rr_err_l2_upper = np.nan
else:
    sigma_rr_err_l2_lower = projection_debug_results[primary_projection_method][
        "sigma_rr_l2_norm_lower"
    ]
    sigma_rr_err_l2_upper = projection_debug_results[primary_projection_method][
...
metrics = {
    ...
    "projection_debug_primary_method": np.bytes_(primary_projection_method or "none"),
}

for method_name, method_result in projection_debug_results.items():
    metric_prefix = f"projection_debug_{method_name}"
    metrics[f"{metric_prefix}_label"] = np.bytes_(method_result["label"])
    metrics[f"{metric_prefix}_lmoresi_requested"] = method_result["lmoresi_requested"]
    metrics[f"{metric_prefix}_projection_seconds"] = method_result["projection_seconds"]
    metrics[f"{metric_prefix}_sigma_metric_seconds"] = method_result[
        "sigma_metric_seconds"
    ]
    metrics[f"{metric_prefix}_rss_max_mib_before"] = method_result[
        "rss_max_mib_before"
    ]
    metrics[f"{metric_prefix}_rss_max_mib_after_projection"] = method_result[
        "rss_max_mib_after_projection"
    ]
    metrics[f"{metric_prefix}_rss_max_mib_after_metrics"] = method_result[
        "rss_max_mib_after_metrics"
    ]
    metrics[f"{metric_prefix}_rss_sum_mib_after_metrics"] = method_result[
        "rss_sum_mib_after_metrics"
    ]
    metrics[f"{metric_prefix}_sigma_rr_l2_norm_lower"] = method_result[
        "sigma_rr_l2_norm_lower"
    ]
    metrics[f"{metric_prefix}_sigma_rr_l2_norm_upper"] = method_result[
        "sigma_rr_l2_norm_upper"
    ]
...
if uw.mpi.rank == 0:
    ...
    for key, value in metrics.items():
        print(f"{key}: {value}")
    print("=== Projection Debug Summary ===")
    print(
        "method,lmoresi_requested,projection_seconds,sigma_metric_seconds,"
        "rss_max_mib_after_metrics,sigma_rr_lower,sigma_rr_upper"
    )
    for method_name, method_result in projection_debug_results.items():
        print(
            f"{method_name},"
            f"{method_result['lmoresi_requested']},"
            f"{method_result['projection_seconds']},"
            f"{method_result['sigma_metric_seconds']},"
            f"{method_result['rss_max_mib_after_metrics']},"
            f"{method_result['sigma_rr_l2_norm_lower']},"
            f"{method_result['sigma_rr_l2_norm_upper']}"
        )
