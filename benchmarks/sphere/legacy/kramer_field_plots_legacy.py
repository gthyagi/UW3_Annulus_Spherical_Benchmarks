# %% [markdown]
# ## Spherical Kramer Legacy Post-Processing
#
# Edit `dirname` below, then run this script to recreate the spherical Kramer
# field plots from the legacy checkpoint output.
...
# %%
dirname = "case2_inv_lc_4_l_2_m_1_k_3_vdeg_2_pdeg_1_pcont_true_vel_penalty_1e+08_stokes_tol_1e-10_ncpus_8"
...
with h5py.File(h5_path, "r") as h5f:
    ...
    v_u = read_field(h5f, "V_u", points.shape[0])
    p_u = read_field(h5f, "P_u", points.shape[0]).reshape(-1)
    v_a = read_field(h5f, "V_a", points.shape[0])
    p_a = read_field(h5f, "P_a", points.shape[0]).reshape(-1)
    v_e = read_field(h5f, "V_e", points.shape[0])
    p_e = read_field(h5f, "P_e", points.shape[0]).reshape(-1)
    rho_a = read_field(h5f, "RHO_a", points.shape[0]).reshape(-1)
