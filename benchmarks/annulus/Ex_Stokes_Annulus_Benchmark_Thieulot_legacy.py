...
res = uw.options.getInt("res", default=2)
cellsize = 1/res
...
if analytical:
    with mesh.access(v_ana, p_ana, rho_ana):
        if k==0:
            ...
            p_ana.data[:,0] = uw.function.evaluate(p.subs({r:r_uw, theta:th_uw}), p_ana.coords)
            rho_ana.data[:,0] = uw.function.evaluate(rho.subs({r:r_uw, theta:th_uw}), rho_ana.coords)
            
        v_ana.data[:,0] = uw.function.evaluate(v_ana_expr[0], v_ana.coords)
        v_ana.data[:,1] = uw.function.evaluate(v_ana_expr[1], v_ana.coords)
...
stokes = Stokes(mesh, velocityField=v_uw, pressureField=p_uw, degree=max(pdegree, vdegree)) # solver_name="stokes",
...
if uw.mpi.size == 1 and additional_analysis:
    # get indices
...
if analytical and uw.mpi.size == 1 and additional_analysis:
