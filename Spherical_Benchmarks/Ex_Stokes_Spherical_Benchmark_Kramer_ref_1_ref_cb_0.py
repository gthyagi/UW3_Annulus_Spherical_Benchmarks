...
msh_h5_load = True # Load existing mesh h5 file 
...
res = 32 # 4, 8, 16, 32, 64, 96 (actual res of the model)
refine = 1
...
# +
# output dir
output_dir = os.path.join(os.path.join("/scratch/m18/tg7098/Spherical_Benchmark_Kramer/Test_Refinement/"),
...
mesh_ref_0 = uw.meshing.SphericalShell(radiusInner=r_i, radiusOuter=r_o, cellSize=cellsize, qdegree=max(pdegree, vdegree), 
                                       filename=f'{output_dir}uw_spherical_shell_ro{r_o}_ri{r_i}_res{res_1}_refineNone.msh', 
                                       refinement=None)

# mesh with refinement
mesh_ref_1 = uw.discretisation.Mesh(f'{output_dir}uw_spherical_shell_ro{r_o}_ri{r_i}_res{res_1}_refineNone.msh.h5', 
                                    degree=1, qdegree=max(pdegree, vdegree), coordinate_system_type=uw.coordinates.CoordinateSystemType.SPHERICAL,
                                    useMultipleTags=True, useRegions=True, markVertices=True, boundaries=boundaries, boundary_normals=boundary_normals,
                                    refinement=refine, refinement_callback=None, verbose=0)

# saving refined mesh
mesh_ref_1.write(filename=f'{output_dir}uw_spherical_shell_ro{r_o}_ri{r_i}_res{res_1}_refine{refine}.h5')
mesh_ref_1.generate_xdmf(filename=f'{output_dir}uw_spherical_shell_ro{r_o}_ri{r_i}_res{res_1}_refine{refine}.h5')
# -
...
# +
if msh_h5_load:
    mesh = uw.discretisation.Mesh(f'{output_dir}uw_spherical_shell_ro{r_o}_ri{r_i}_res{res_1}_refine{refine}.h5', 
                                  degree=1, qdegree=max(pdegree, vdegree), coordinate_system_type=CoordinateSystemType.SPHERICAL,
                                  useMultipleTags=True, useRegions=True, markVertices=True, boundaries=boundaries, boundary_normals=boundary_normals,
