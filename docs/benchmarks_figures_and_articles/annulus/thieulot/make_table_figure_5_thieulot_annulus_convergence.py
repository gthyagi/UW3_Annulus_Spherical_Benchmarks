...
OUTPUT_FRAGMENT = OUTPUT_DIR / "table_figure_5_thieulot_annulus_convergence_fragment.tex"
OUTPUT_RATE_FRAGMENT = OUTPUT_DIR / "table_observed_convergence_rates_fragment.tex"
...
ELEMENT_THEORY = {
    (1, 0, False): ("unstable", "-/-"),
    (1, 1, True): ("non-standard", "-/-"),
    (2, 0, False): ("non-standard", "2/1"),
    (2, 1, False): ("non-standard", "-/-"),
    (2, 1, True): ("stable", "3/2"),
    (3, 2, True): ("stable", "4/3"),
}
...
def fmt_rate(value: float | None, ndigits: int = 2) -> str:
...
def fitted_rate(series: list[MetricRecord], metric_name: str) -> float | None:
    h = np.array([record.h for record in series], dtype=float)
    y = np.array([getattr(record, metric_name) for record in series], dtype=float)
...
def observed_rate_summary(records: list[MetricRecord]) -> str:
    lines = [
        r"% Generated by make_table_figure_5_thieulot_annulus_convergence.py.",
        r"\begin{table}[H]",
        r"\centering",
        r"\caption{Observed convergence rates for the three annulus benchmark cases with analytical solutions. The optimal column gives the expected smooth-solution rates used here as velocity/pressure; \(-/-\) denotes element pairs for which no optimal-rate claim is made here. Observed entries are least-squares slopes fitted to \(\log E\) versus \(\log h\) and are reported as \(E_{L_2}^{\mathrm{rel}}(\mathbf{u})/E_{L_2}^{\mathrm{rel}}(p)\).}",
        r"\label{tab:observed_convergence_rates}",
        r"\small",
        r"\begin{tabular}{lp{0.20\linewidth}cccc}",
        r"\toprule",
        r"Element pair & Status & Optimal & \(k=1\) & \(k=4\) & \(k=8\) \\",
        r"\midrule",
    ]

    for vdegree, pdegree, pcont, label in ELEMENTS:
        stable, optimal = ELEMENT_THEORY[(vdegree, pdegree, pcont)]
        row = [label, stable, optimal]
        for k in K_VALUES:
            series = values_for_series(records, vdegree, pdegree, pcont, k)
            v_rate = fitted_rate(series, "v_l2_norm")
            p_rate = fitted_rate(series, "p_l2_norm")
            row.append(rf"{fmt_rate(v_rate, 1)}/{fmt_rate(p_rate, 1)}")
...
def table_lines(records: list[MetricRecord], element: tuple[int, int, bool, str]) -> list[str]:
    ...
    lines = [
        ...
        r"$h$ & $E_{L_2}^{\mathrm{rel}}(\mathbf{u})$ & rate & $E_{L_2}^{\mathrm{rel}}(p)$ & rate & $E_{L_2}^{\mathrm{rel}}(\mathbf{u})$ & rate & $E_{L_2}^{\mathrm{rel}}(p)$ & rate & $E_{L_2}^{\mathrm{rel}}(\mathbf{u})$ & rate & $E_{L_2}^{\mathrm{rel}}(p)$ & rate \\",
        r"\midrule",
    ]
...
def table_block(records: list[MetricRecord], element: tuple[int, int, bool, str]) -> str:
    ...
    lines = [
        ...
        *table_lines(records, element),
        r"\end{table}",
    ]
    return "\n".join(lines)


def article_fragment(records: list[MetricRecord]) -> str:
    lines = [
        r"% Generated by make_table_figure_5_thieulot_annulus_convergence.py.",
        r"\par\nolinenumbers",
    ]
    ...
    for idx, element in enumerate(ELEMENTS):
        vdegree, pdegree, pcont, label = element
        if idx % 2 == 0:
            lines.extend([r"\begin{landscape}", r"\thispagestyle{empty}"])

        lines.extend(
            [
                r"\begin{center}",
                r"\centering",
                rf"\captionof{{table}}{{Relative errors and convergence rates for {label}.}}",
                rf"\label{{tab:annulus_thieulot_v{vdegree}_p{pdegree}_{'cont' if pcont else 'disc'}}}",
                *table_lines(records, element),
                r"\end{center}",
            ]
        ...
        if idx % 2 == 1:
            lines.extend(
                [
                    r"\vfill",
                    r"\begin{center}\thepage\end{center}",
                ]
            )
    ...
    lines.append(r"\par\linenumbers")
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    OUTPUT_FRAGMENT.write_text(article_fragment(records))
    OUTPUT_RATE_FRAGMENT.write_text(observed_rate_summary(records))
    print(f"Wrote {OUTPUT_TEX}")
    print(f"Wrote {OUTPUT_FRAGMENT}")
    print(f"Wrote {OUTPUT_RATE_FRAGMENT}")
